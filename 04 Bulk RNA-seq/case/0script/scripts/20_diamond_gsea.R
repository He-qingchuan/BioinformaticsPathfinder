# 文件用途｜第 20 章：批量运行 DIAMOND 自建注释的非模式 GSEA。
...
# 接口｜run_diamond_gsea：批量运行 DIAMOND 自建注释的非模式 GSEA。
...
#   的矢量图形不靠此值提高清晰度。
# output_root（默认 "9Enrichment_Analysis/OrgDb/diamond/GSEA"）：字符路径；本步骤输出根目录，函数再按
#   比较/参数建立子目录。通常沿用；试算可选新目录，但后续读取位置不会自动更新。
# variant_label（默认 ""）：字符标签；空字符串为默认路径。给次要排版或另一方案另存时用 large_font 等简
#   短标签，仅允许字母数字及 . _ -，首位为字母数字；不能放路径或空格。
# 返回/写出与边界：单独保存支线结果与图，使用既有注释，不重新做蛋白搜索。
run_diamond_gsea <- function(comparison_ids = NULL,
  ...
  # 功能：载入主 GSEA、自建注释和本章调用函数，保留指定串行后端。
  # 首次先执行第 15、19、20 章相应函数定义，不必运行它们的批量任务。
  library(tidyverse)
  ...
  source("0script/scripts/20_nonmodel_gsea_functions.R", local = TRUE)
  readRenviron("0script/project.env")

  # 功能：设置本支线的 GSEA 范围及展示参数；默认保留两种排序和 Top10/Top20。
  if (is.null(comparison_ids)) comparison_ids <- NULL             # 所有启用比较；可从 contrasts.tsv 选择一个或多个 ID。

  # 功能：完整传递参数；函数内部不会重新读取环境阈值覆盖这些设置。
  plot_nonmodel_GSEA(method = "diamond", comparison_ids = comparison_ids, rankings = rankings,
    term_num = term_num, ontologies = ontologies, include_kegg = include_kegg,
    fdr_cutoff = fdr_cutoff, minGSSize = minGSSize, maxGSSize = maxGSSize,
    simplify_cutoff = simplify_cutoff, seed = seed,
    draw_dotplot = draw_dotplot, pvalue_tables = pvalue_tables,
    plot_dpi = plot_dpi, output_root = output_root, variant_label = variant_label)
