# 文件用途｜第 15 章：选择启用列表中的第二个比较（不足两个时取第一个）做模式 GSEA 诊断。
...
# 接口｜run_gsea_example_diagnostics：选择启用列表中的第二个比较（不足两个时取第一个）做模式 GSEA 诊断。
# 参数：无显式形参；所需上层对象/输入见功能说明及下方读取语句。
# 返回/写出与边界：对该比较的 log2FoldChange/stat 两种排序的 GO_BP 调用底层复算诊断；不是遍历所有比较
#   ，原富集结果不覆盖。
run_gsea_example_diagnostics <- function() {
  ...
  for (ranking in c("log2FoldChange", "stat")) {
    diagnose_saved_GSEA("main", diagnostic_id, ranking, "GO_BP")
  }
}
