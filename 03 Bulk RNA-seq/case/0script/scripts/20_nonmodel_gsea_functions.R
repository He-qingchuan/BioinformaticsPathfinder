# 文件用途｜第 20 章：复用通用 GSEA 实现并传入自建 GO/通路对应。
...
# 接口｜plot_nonmodel_GSEA：复用通用 GSEA 实现并传入自建 GO/通路对应。
...
# 返回/写出与边界：按比较与排序保存支线 GSEA；保留完整排序，缺自建映射不能自动借用另一物种。
plot_nonmodel_GSEA <- function(method = "diamond", comparison_ids = NULL,
                                rankings = c("log2FoldChange", "stat"), term_num = c(10, 20),
                                ontologies = c("BP", "MF", "CC", "ALL"), include_kegg = TRUE,
                                fdr_cutoff = 0.05, minGSSize = 10, maxGSSize = 500,
                                simplify_cutoff = 0.75, seed = 20260907,
  ...
  stopifnot(length(comparison_ids) > 0, all(comparison_ids %in% comparisons$contrast_id),
            length(rankings) > 0, all(rankings %in% names(de_result)))
  if (is.null(output_root)) output_root <- file.path(annotation$route_dir, "GSEA")
  for (id in comparison_ids) {
    for (ranking_column in rankings) {
      plot_GSEA(de_result = de_result, contrast_id = id, ranking_column = ranking_column, OrgDb_name = annotation$OrgDb,
        keyType = "GID", organism_name = NULL, output_root = output_root,
        fdr_cutoff = fdr_cutoff, term_num = term_num, ontologies = ontologies, include_kegg = include_kegg,
        minGSSize = minGSSize, maxGSSize = maxGSSize, simplify_cutoff = simplify_cutoff,
        draw_dotplot = draw_dotplot, pvalue_tables = pvalue_tables, plot_dpi = plot_dpi,
        kegg_terms = annotation$TERM2GENE, kegg_names = annotation$TERM2NAME,
        seed = seed, variant_label = variant_label)
    }
  }
}
