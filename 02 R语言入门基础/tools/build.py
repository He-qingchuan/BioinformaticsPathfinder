...
def main():
    ...
    archive(ROOT/'practice/r-lab.zip',[(p,'r-lab/'+p.relative_to(lab).as_posix()) for p in lab.rglob('*') if p.is_file() and not {'.Rproj.user','.cache','.Rhistory','.RData'}.intersection(p.relative_to(lab).parts) and ('results' not in p.relative_to(lab).parts or p.name=='README.md')])
